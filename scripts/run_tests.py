...
def run_all_tests():
    ...
    if os.path.exists(search_path):
        for root, dirs, files in os.walk(search_path):
            for file in files:
                ...
                if file.startswith("messengerTests") and is_executable(full_path):
    ...
    for i, binary in enumerate(test_binaries):
        if i > 0:
            delay = 5
            color_print(
                f"\n[INFO] Waiting {delay} seconds for OS to release ports (TIME_WAIT)...",
                Colors.HEADER,
            )
            time.sleep(delay)

        code = run_single_binary(binary)
        results.append(code)
